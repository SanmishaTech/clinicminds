...
export async function POST(req: NextRequest) {
  ...
  const { patientId, discountPercent = 0, totalAmount, medicineBillDetails, receipt } = parsed.data;
  ...
  if (!franchiseId) {
    throwHttp('Current user is not associated with any franchise', 400);
      ...
      if (receipt && receipt.amount && receipt.amount > 0) {
        // Get patient info for balance update
        const patient = await tx.patient.findUnique({
          where: { id: patientId },
          select: { balanceAmount: true }
        });

        const medicineBillReceipt = await tx.medicineBillReceipt.create({
          data: {
            ...
            chequeDate: receipt.chequeDate ? new Date(receipt.chequeDate) : null,
            notes: receipt.notes || '',
            createdByUserId: auth.user.id,
          },
        });

        if (medicineBillReceipt) {
          // Update medicine bill's totalReceivedAmount
          await tx.medicineBill.update({
            where: { id: medicineBill.id },
            data: {
              totalReceivedAmount: receipt.amount
            }
          });

          // Update patient's balanceAmount (add medicine bill amount, subtract receipt if provided)
          if (patient) {
            const currentBalanceAmount = Number(patient.balanceAmount || 0);
            const newBalanceAmount = receipt.amount 
              ? currentBalanceAmount + finalAmount - receipt.amount
              : currentBalanceAmount + finalAmount;
          ...
          // No receipt - just add medicine bill amount to patient balance
          if (patient) {
            const currentBalanceAmount = Number(patient.balanceAmount || 0);
            await tx.patient.update({
              where: { id: patientId },
              data: {
                balanceAmount: currentBalanceAmount + finalAmount
              }
            });
          }
        }
