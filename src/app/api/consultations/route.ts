  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      if (data.consultationMedicines && data.consultationMedicines.length > 0) {
        ...
        // Update patient's balanceAmount (add consultation amount, subtract receipt if provided)
        const appointment = await tx.appointment.findUnique({
          where: { id: data.appointmentId },
          select: { patientId: true }
        });
        ...
        if (appointment?.patientId) {
          ...
          if (patient) {
            ...
            const newBalanceAmount = data.receipt?.amount 
              ? currentBalanceAmount + data.totalAmount - data.receipt.amount
              : currentBalanceAmount + data.totalAmount;
