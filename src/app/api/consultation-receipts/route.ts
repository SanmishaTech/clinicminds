...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Calculate balance amount and validate receipt amount doesn't exceed it
    const totalAmount = Number(consultation.totalAmount);
    const totalReceivedAmount = Number(consultation.totalReceivedAmount || 0);
    const balanceAmount = totalAmount - totalReceivedAmount;
    
    if (validatedData.amount > balanceAmount) {
      return BadRequest(`Receipt amount (${validatedData.amount}) cannot exceed balance amount (${balanceAmount})`);
    }
