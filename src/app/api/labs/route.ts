...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return Error('Failed to fetch labs');
  }
}
...
export async function POST(req: NextRequest) {
  ...
  if (!name || String(name).trim() === '') return Error('Lab name is required', 400);
  if (String(name).trim().length > 100) return Error('Lab name must be less than 100 characters', 400);
  ...
  try {
    ...
    if (err?.code === 'P2002') return Error('Lab already exists', 409);
    console.error('Create lab error:', e);
    return Error('Failed to create lab');
  }
}

// PATCH /api/labs  { id, name? }
export async function PATCH(req: NextRequest) {
  ...
  if (!id) return Error('Lab id required', 400);

  const data: Record<string, unknown> = {};
  if (typeof name === 'string' || name === null) {
    if (!name || name.trim() === '') return Error('Lab name is required', 400);
    if (name.trim().length > 100) return Error('Lab name must be less than 100 characters', 400);
    data.name = name.trim();
  }
