...
const ROLE_CODES = ACTIVE_ROLE_CODES as readonly string[];
function toRoleCode(v?: string) {
	if (!v) return undefined as unknown as any;
	if (ROLE_CODES.includes(v)) return v as any;
	const found = ROLE_ENTRIES.find(([, label]) => label === v);
	const code = found?.[0];
	return code && ROLE_CODES.includes(code) ? (code as any) : (undefined as unknown as any);
}
...
export function UserForm({
	...
	const ROLE_VALUES = ACTIVE_ROLE_CODES as unknown as [string, ...string[]];
	const roleOptions = (ACTIVE_ROLE_CODES as readonly string[]).map((code) => ({
		value: code,
		label: (ROLES as any)[code] ?? code,
	}));
